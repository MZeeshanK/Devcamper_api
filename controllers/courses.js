...
exports.updateCourse = asyncHandler(async (req, res, next) => {
  ...
  if (course.user.toString() !== req.user.id && req.user.role !== 'admin') {
    return next(
      new ErrorResponse(
        `User ${req.params.id} is not authorized to update a course${course._id}`
      )
    );
  }
...
exports.deleteCourse = asyncHandler(async (req, res, next) => {
  ...
  if (course.user.toString() !== req.user.id && req.user.role !== 'admin') {
    return next(
      new ErrorResponse(
        `User ${req.params.id} is not authorized to delete a course${course._id}`
      )
    );
  }
  ...
  res.status(200).json({ success: true, courseDelete: course.title });
});
